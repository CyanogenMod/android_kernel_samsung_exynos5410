 ...
 */
	{1, 0, 0, 1, 1, 1, 7},	/* L0(For ISP) */
	{1, 0, 2, 1, 1, 1, 7},	/* L1(For ISP) */
	{1, 7, 7, 1, 1, 1, 7},	/* L2 */
	{1, 7, 7, 1, 1, 1, 7},	/* L3 */
	{4, 7, 7, 1, 1, 1, 7},	/* L4 */
	{5, 7, 7, 1, 1, 1, 7},	/* L5 */
				      ...
				      unsigned long *_freq, u32 flags)
{
	...
	if (exynos5410_int_limit_freq < int_bus_opp_list[0].clk)
		*_freq = min((unsigned int)*_freq, exynos5410_int_limit_freq);
...
static int exynos5410_init_int_table(struct busfreq_data_int *data)
{
	...
	exynos5410_int_limit_freq = int_bus_opp_list[0].clk;
						...
						unsigned long event, void *v)
{
	...
	case TMU_95:
		...
		break;
	case TMU_111:
		exynos5410_int_limit_freq = INT_PANIC_FREQ;
		schedule_delayed_work(&do_panic, HZ);
		break;
	}
...
static void exynos_int_do_panic(struct work_struct *work)
{
	pr_info("%s is called\n", __func__);

	if (exynos_int_cur_freq > INT_PANIC_FREQ) {
		pr_info("exynos_int_cur_freq is %d\n", exynos_int_cur_freq);
		goto out;
	}

	panic("FIMC TEST case is occur\n");
out:
	schedule_delayed_work(&do_panic, HZ);
}
...
static __devinit int exynos5_busfreq_int_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&do_panic, exynos_int_do_panic);

	exynos_int_cur_freq = int_bus_opp_list[2].clk;
