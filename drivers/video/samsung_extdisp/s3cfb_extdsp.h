...
#define S3CFB_EXTDSP_GET_FB_PHY_ADDR		_IOR('F', 310, \
						struct s3cfb_extdsp_time_stamp)
...
#define S3CFB_EXTDSP_PUT_FD		        _IOW('F', 323, \
						struct s3cfb_extdsp_time_stamp)
...
extern int s3cfb_extdsp_mmap(struct fb_info *info, struct vm_area_struct *vma);
extern int s3cfb_extdsp_pan_display(struct fb_var_screeninfo *var, struct fb_info *info);
